          ...
          <div className="leaderboard-list">
            {filteredItems.map((item, index) => (
              <LeaderboardRow
                ...
                userVote={userVotes?.[item._id] ?? null}
              />
            ))}
          </div>
        )}
      ...
      <aside className="sidebar sidebar-right">
        {trending && trending.length > 0 && (
          <div className="sidebar-section">
            ...
            <div className="sidebar-category-list">
              {trending.map((cat) => (
                <Link key={cat._id} to={`/category/${cat.slug}`} className="sidebar-category-item">
                  {cat.name}
                </Link>
              ))}
            </div>
        ...
        {hated && hated.length > 0 && (
          <div className="sidebar-section">
            ...
            <div className="sidebar-category-list">
              {hated.map((cat) => (
                <Link key={cat._id} to={`/category/${cat.slug}`} className="sidebar-category-item hated">
                  {cat.name}
                </Link>
              ))}
            </div>
...
const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ item, rank, categoryId, userVote }) => {
  const { user } = useAuth();
  ...
  const handleVote = async (value: number) => {
    if (!user) {
      // Could redirect to login, for now just alert
  ...
  return (
    <div className={`lb-row ${rank <= 3 ? `lb-top-${rank}` : ""}`}>
      ...
      <div className="lb-votes">
        <button
          className={`vote-arrow vote-up ${userVote === 1 ? "voted" : ""}`}
          title="Upvote"
        ...
        <button
          className={`vote-arrow vote-down ${userVote === -1 ? "voted" : ""}`}
          title="Downvote"
          onClick={() => handleVote(-1)}
          disabled={voting}
        >
